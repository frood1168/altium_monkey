        ...
        config = OutJobConfigRecord(
            ...
            fields=(
                ...
                (
                    ...
                    _bool_str(self.optimize_change_location_commands),
                ),
...
class NCDrillOutput:
    ...
    def to_spec(self) -> OutJobOutputSpec:
        ...
        if self.generate_separate_via_type_files is not None:
            fields.append(
                (
                    "GenerateSeparateViaTypeFiles",
                    _bool_str(self.generate_separate_via_type_files),
                )
            )
        if self.generate_tools_by_drill_symbols is not None:
            fields.append(
                (
                    "GenerateToolsByDrillSymbols",
                    _bool_str(self.generate_tools_by_drill_symbols),
                )
            )
        fields.extend(
            (
    ...
    def to_spec(self) -> OutJobOutputSpec:
        ...
        config = OutJobConfigRecord(
            ...
            fields=(
                ...
                (
                    "NoERCSymbolsToShow",
                    '"Thin Cross","Thick Cross","Small Cross",Checkbox,Triangle',
                ),
                ("ShowNote", "True"),
...
class AltiumOutJob:
    ...
    def create_pcb_fabrication(
        ...
        *,
        ...
        if gerber:
            mid = outjob.add_generated_files_medium("GERBERS", f"{output_dir}/gerbers")
            outjob.add_gerber(pcbdoc_path, enabled_medium=mid)

        if nc_drill:
            mid = outjob.add_generated_files_medium("DRILLS", f"{output_dir}/drills")
            outjob.add_nc_drill(pcbdoc_path, enabled_medium=mid)

        if odb:
            mid = outjob.add_generated_files_medium("ODB++", f"{output_dir}/odb")
    ...
    def add_publish_medium(
        ...
        *,
        ...
        self.config.set(
            ps, f"UseOutputNameForMulti{idx}", "1" if filename_multi else "0"
        )
    ...
    def _add_output_type(
        ...
        *,
        ...
        if config_items:
            ...
            self.config.set(
                sec, f"Configuration{idx}_Name1", "OutputConfigurationParameter1"
            )
    ...
    def set_generated_files_path(self, medium_index: int, output_dir: str) -> None:
        ...
        output_dir = _normalize_path(output_dir, is_directory=True)
        self.config.set(
            "GeneratedFilesSettings", f"RelativeOutputPath{medium_index}", output_dir
        )
    ...
    def set_publish_path(
        ...
        *,
        ...
        if filename_multi is not None:
            ...
            self.config.set(
                ps,
                f"UseOutputNameForMulti{medium_index}",
                "1" if filename_multi else "0",
            )
        if filename_special is not None:
            self.config.set(
                ps, f"OutputFileNameSpecial{medium_index}", filename_special
            )
            ...
            result.append(
                {
                    ...
                    "category": self.config.get(
                        sec, f"OutputCategory{idx}", fallback=""
                    ),
                    "enabled": self.config.get(
                        sec, f"OutputEnabled{idx}", fallback="0"
                    ),
                }
            )
            idx += 1
            ...
            result.append(
                {
                    ...
                    "type": self.config.get(
                        sec, f"OutputMedium{idx}_Type", fallback=""
                    ),
                    "index": idx,
                }
            )
    ...
    def __repr__(self) -> str:
        return (
            f"AltiumOutJob(mediums={self._medium_count}, outputs={self._output_count})"
        )
